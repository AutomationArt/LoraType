...
/* Display resolution */
#define EPD_WIDTH_290          128
#define EPD_HEIGHT_290         296
 ...
 */
void DEPG0290BxS75AFxX_BW::SetFrameMemory(
    ...
    x &= 0xF8;
    image_width &= 0xF8;
    if (x + image_width >= EPD_WIDTH_290) {
        x_end = EPD_WIDTH_290 - 1;
    } else {
        x_end = x + image_width - 1;
    }
    if (y + image_height >= EPD_HEIGHT_290) {
        y_end = EPD_HEIGHT_290 - 1;
    } else {
 ...
 *          this won't update the display.
 */
void DEPG0290BxS75AFxX_BW::ClearFrameMemory(unsigned char color) {

    SendCommand(WRITE_RAM);
    /* send the color data */
    for (int i = 0; i < EPD_WIDTH_290 / 8 * EPD_HEIGHT_290; i++) {
        SendData(color);
    }
}
/**
 ...
 */
void DEPG0290BxS75AFxX_BW::HalLcd_Partial_Update(void) {
 ...
 */
void DEPG0290BxS75AFxX_BW::SetMemoryArea(int x_start, int y_start, int x_end, int y_end) {
    SendCommand(0x11); //data entry mode
    SendData(0x03);
    SendCommand(0x44);
 ...
 */
void DEPG0290BxS75AFxX_BW::SetMemoryPointer(int x, int y) {
...
void DEPG0290BxS75AFxX_BW::Clear(void)
{
    int w, h;
    w = (EPD_WIDTH_290 % 8 == 0)? (EPD_WIDTH_290 / 8 ): (EPD_WIDTH_290 / 8 + 1);
    h = EPD_HEIGHT_290;
